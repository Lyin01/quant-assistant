...
if page == "总览":
    ...
    st.subheader("市场连涨趋势扫描")
    st.caption("按成交额优先扫描流动性较高的 ETF / A股，基于趋势结构、上涨连续性、动量、回撤、RSI、MACD、量能确认打分。")
    scan_col1, scan_col2, scan_col3, scan_col4 = st.columns([1.2, 1, 1.2, 2.2])
    universe_label_to_key = {label: key for key, label in MARKET_UNIVERSES.items()}
    universe_label = scan_col1.radio(
        "扫描范围",
        options=list(universe_label_to_key.keys()),
        index=0,
        horizontal=True,
    )
    scan_universe = universe_label_to_key.get(universe_label, "etf")
    scan_limit = scan_col2.number_input("扫描数量", min_value=10, max_value=500, value=DEFAULT_SCAN_LIMIT, step=10)
    mode_label_to_key = {label: key for key, label in SCAN_MODES.items()}
    mode_label = scan_col3.radio("选股模式", options=list(mode_label_to_key.keys()), index=1, horizontal=True)
    scan_mode = mode_label_to_key.get(mode_label, "balanced")
    include_defensive = scan_col4.checkbox("包含货币/债券类 ETF", value=False)
    scan_col4.caption("默认扫成交额前 30 个标的；A股建议先扫 30-100。稳健模式更重视不过热和量能确认，进攻模式更重视动量。")
    btn_col1, btn_col2 = scan_col4.columns([1, 1])
    if btn_col1.button("开始扫描", type="primary"):
        with st.spinner(f"正在扫描 {universe_label} 中成交额前 {scan_limit} 个标的..."):
            scan_result, scan_messages = scan_market(
        ...
        with st.spinner(f"正在扫描 {universe_label} 中成交额前 {scan_limit} 个标的...（跳过缓存）"):
            scan_result, scan_messages = scan_market(
                universe=scan_universe,
                top_n=scan_limit,
                force_refresh=True,
    ...
    if scan_result is not None:
        if scan_result.empty:
            ...
            display_cols = [
                "排名", "类型", "代码", "名称", "价格", "候选分", "连涨趋势分", "趋势等级",
                "买入建议", "持有/卖出建议", "观察建议", "风险提示",
                "5日涨幅%", "10日涨幅%", "20日涨幅%", "5日上涨天数", "10日上涨天数",
                "连续上涨天数", "MA20斜率%", "RSI", "20日回撤%", "量比", "成交额",
            ]
