...
function parseArgs(argv) {
  ...
  return {
    universe: String(args.get("universe") || "etf"),
...
async function fetchList(universe) {
  const fs =
    universe === "etf"
  ...
  return rows
    ...
    .filter((row) => row.code && row.name && Number.isFinite(row.price) && Number.isFinite(row.amount))
    .filter((row) => universe === "etf" || /^(00|30|60|68)\d{4}$/.test(row.code));
}

async function fetchUniverse(universe) {
  let rows;
  try {
    if (universe === "all") {
  ...
  return rows.filter((row) => {
    ...
    if (row.assetType === "ETF" && !globalThis.__includeDefensive && DEFENSIVE_ETF_KEYWORDS.some((word) => row.name.includes(word))) return false;
    return true;
  });
}

async function cachedUniverse(universe) {
...
function compute(row, klines) {
  if (klines.length < 25) return null;
  const close = klines.map((k) => k.close);
  const volume = klines.map((k) => k.volume);
  const last = close.at(-1);
  const ret = (days) => (close.length > days ? (last / close[close.length - 1 - days] - 1) * 100 : 0);
  const pct = close.map((value, index) => (index === 0 ? 0 : (value / close[index - 1] - 1) * 100));
  const ma = (window, offset = 0) => avg(close.slice(close.length - window - offset, close.length - offset));
  const ma20 = ma(20);
  ...
  const factors = { ret5: ret(5), ret10: ret(10), ret20: ret(20), trendScore, upDays5, upDays10, consecutiveUpDays, drawdown20, rsi, volRatio, volatility };
  const score = continuationScore(factors);
  const candidate = candidateScore(score, factors, row.amount, row.assetType, globalThis.__scanMode || "balanced");
  return {
    type: row.assetType,
    code: row.code,
    name: row.name,
...
function candidateScore(base, f, amount, assetType, mode) {
...
function liquidityBonus(amount) {
...
function riskPenalty(f, assetType, mode) {
  let penalty = 0;
...
function riskNote(f, amount, assetType) {
  const notes = [];
...
function tradeSuggestion(f, assetType) {
...
function sellSuggestion(f, assetType) {
  const score = f.candidateScore || f.continuationScore || 0;
  const drawdown = f.drawdown20 || 0;
  const rsi = f.rsi || 50;
  const ret20 = f.ret20 || 0;
...
function actionNote(score, drawdown20, rsi) {
...
async function cachedScanRow(row) {
  const cacheDir = path.join(process.cwd(), "data", "cache", "generic");
...
async function main() {
  ...
  globalThis.__includeDefensive = args.includeDefensive;
  globalThis.__scanMode = args.mode;
  const universeRows = await fetchUniverse(args.universe);
  const selected = universeRows.sort((a, b) => b.amount - a.amount).slice(0, args.topN);
  const scanned = await mapLimit(selected, args.workers, async (row) => compute(row, await fetchKlines(row.code)));
